...
-- Them khoa ngoai cho public.posts
ALTER TABLE public.posts ADD CONSTRAINT fk_post FOREIGN KEY(userId) REFERENCES public.users(id);

-- Them khoa ngoai cho public.users
ALTER TABLE public.users ADD CONSTRAINT fk_user FOREIGN KEY(roleId) REFERENCES public.roles(id);

-- Them khoa ngoai cho public.reviews
ALTER TABLE public.reviews ADD CONSTRAINT fk01_review FOREIGN KEY(userId) REFERENCES public.users(id);
ALTER TABLE public.reviews ADD CONSTRAINT fk02_review FOREIGN KEY(productId) REFERENCES public.products(id);

-- Them khoa ngoai cho public.products
ALTER TABLE public.products ADD CONSTRAINT fk_product FOREIGN KEY(categoryId) REFERENCES public.categories(id);

-- Them khoa ngoai cho public.images
ALTER TABLE public.images ADD CONSTRAINT fk_image FOREIGN KEY(productId) REFERENCES public.products(id);

-- Them khoa ngoai cho public.product_variants
ALTER TABLE public.product_variants ADD CONSTRAINT fk_product_variant FOREIGN KEY(productId) REFERENCES public.products(id);

-- Them khoa ngoai cho public.reservation_products
ALTER TABLE public.reservation_products ADD CONSTRAINT fk01_reservation_product FOREIGN KEY(productId) REFERENCES public.products(id);
ALTER TABLE public.reservation_products ADD CONSTRAINT fk02_reservation_product FOREIGN KEY(reservId) REFERENCES public.reservations(id);

-- Them khoa ngoai cho public.reservations
ALTER TABLE public.reservations ADD CONSTRAINT fk01_reservation FOREIGN KEY(userId) REFERENCES public.users(id);
ALTER TABLE public.reservations ADD CONSTRAINT fk02_reservation FOREIGN KEY(tableId) REFERENCES public.tables(id);

-- Them khoa ngoai cho public.tables
ALTER TABLE public.tables ADD CONSTRAINT fk01_information_table FOREIGN KEY(areaId) REFERENCES public.areas(id);
...
values ('menber', 'Thực hiện chức năng của khách hàng'),
 ...
 -- 2
insert into public.contacts (fullname, email, phone, subject, content)
...
-- 3
insert into public.categories(name, status, image)
...
-- 4
insert into public.areas(name,status)
values ('Sân vườn', 1),
 ('Tầng 1', 1),
 ('Tầng 2', 1),
 ('Tầng 3', 1),
...
-- 5
insert into public.tables (areaId, tableNum, seatCount, location,image,status)
...
-- 6
insert into public.users (roleId, username, passwordUser, email, emailVerify, phone, status, fullname, avatar, createdDate)
...
-- 7
insert into public.posts( userId, title, shortDescription, description, thumbnail, status, createdBy, createdDate)
...
-- 8
insert into public.products (categoryId, name, price, description, status, discount, createdDate)
...
-- image
insert into public.images(productId, url)
values
...
-- 9
insert into public.reservations (userId, tableId,contactName,contactPhone,contactEmail, startTime, endTime, status, paymentMethod,note ,totalPrice,createdDate)
...
-- 10
insert into public.product_variants(productId, size,pricePlus)
values (1, 'S',0),
 (1, 'M',10),
 (1, 'L',20),
 (2, 'S',0),
...
-- 11
insert into public.reservation_products(productId, reservId, quantity, price, size)
values (1, 1, 2, 120, '3'),
 (2, 1, 10, 800, '2'),
 (1, 2, 5, 200, '1'),
 (2, 2, 8, 720, '3');
...
-- 12
insert into public.reviews(productId, userId, starRate, content)
